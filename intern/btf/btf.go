...
var (
	ErrNotSupported   = intern.ErrNotSupported
...
func LoadSpecFromReader(rd io.ReaderAt) (*Spec, error) {
	file, err := intern.NewSafeELFFile(rd)
	if err != nil {
		return nil, err
	}
	defer file.Close()
...
func findBtfSections(file *intern.SafeELFFile) (*elf.Section, *elf.Section, map[string]uint32, error) {
	var (
...
func loadKernelSpec() (*Spec, error) {
	...
	if err == nil {
		...
		return loadNakedSpec(fh, intern.NativeEndian, nil, nil)
	}
	...
	return nil, fmt.Errorf("no BTF for kernel version %s: %w", release, intern.ErrNotSupported)
}
...
func parseBTF(btf io.ReadSeeker, bo binary.ByteOrder) ([]rawType, stringTable, error) {
	...
	if _, err := io.CopyN(intern.DiscardZeroes{}, rd, remainder); err != nil {
...
func NewHandle(spec *Spec) (*Handle, error) {
	...
	if spec.byteOrder != intern.NativeEndian {
		return nil, fmt.Errorf("can't load %s BTF on %s", spec.byteOrder, intern.NativeEndian)
	}

	btf, err := spec.marshal(marshalOpts{
		ByteOrder:        intern.NativeEndian,
	...
	attr := &bpfLoadBTFAttr{
		btf:     intern.NewSlicePointer(btf),
	...
	if err != nil {
		...
		return nil, intern.ErrorWithLog(err, logBuf, logErr)
	}
...
func bpfLoadBTF(attr *bpfLoadBTFAttr) (*intern.FD, error) {
	fd, err := intern.BPF(intern.BPF_BTF_LOAD, unsafe.Pointer(attr), unsafe.Sizeof(*attr))
	if err != nil {
		return nil, err
	}
...
var haveBTF = intern.FeatureTest("BTF", "5.1", func() error {
	var (
		types struct {
			Integer btfType
			Var     btfType
	...
	btf := marshalBTF(&types, strings, intern.NativeEndian)
...
var haveFuncLinkage = intern.FeatureTest("BTF func linkage", "5.6", func() error {
	if err := haveBTF(); err != nil {
		return err
	}
