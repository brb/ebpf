...
func readRecord(rd io.Reader, cpu int) (Record, error) {
	...
	err := binary.Read(rd, intern.NativeEndian, &header)
	if err == io.EOF {
...
func readLostRecords(rd io.Reader) (uint64, error) {
	...
	err := binary.Read(rd, intern.NativeEndian, &lostHeader)
	if err != nil {
...
func readRawSample(rd io.Reader) ([]byte, error) {
	...
	var size uint32
	if err := binary.Read(rd, intern.NativeEndian, &size); err != nil {
...
func (pr *Reader) Close() error {
	...
	pr.closeOnce.Do(func() {
		...
		var value [8]byte
		intern.NativeEndian.PutUint64(value[:], 1)
