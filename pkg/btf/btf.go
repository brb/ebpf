...
var (
	ErrNotSupported   = pkg.ErrNotSupported
...
func LoadSpecFromReader(rd io.ReaderAt) (*Spec, error) {
	file, err := pkg.NewSafeELFFile(rd)
	if err != nil {
		return nil, err
	}
	defer file.Close()
...
func loadSpecFromELF(file *pkg.SafeELFFile, variableOffsets map[variable]uint32) (*Spec, error) {
	var (
...
func loadKernelSpec() (*Spec, error) {
	...
	if err == nil {
		...
		return LoadRawSpec(fh, pkg.NativeEndian)
	}
	...
	for _, loc := range locations {
		...
		file, err := pkg.NewSafeELFFile(fh)
		if err != nil {
			return nil, err
		}
		defer file.Close()
	...
	return nil, fmt.Errorf("no BTF for kernel version %s: %w", release, pkg.ErrNotSupported)
}
...
func parseBTF(btf io.Reader, bo binary.ByteOrder) ([]rawType, stringTable, error) {
	...
	if _, err := io.CopyN(pkg.DiscardZeroes{}, rd, remainder); err != nil {
...
func NewHandle(spec *Spec) (*Handle, error) {
	...
	if spec.byteOrder != pkg.NativeEndian {
		return nil, fmt.Errorf("can't load %s BTF on %s", spec.byteOrder, pkg.NativeEndian)
	}

	btf, err := spec.marshal(marshalOpts{
		ByteOrder:        pkg.NativeEndian,
	...
	if err != nil {
		...
		attr.logBuf = pkg.NewSlicePointer(logBuf)
		attr.btfLogSize = uint32(len(logBuf))
		attr.btfLogLevel = 1
		_, logErr := bpfLoadBTF(attr)
		return nil, pkg.ErrorWithLog(err, logBuf, logErr)
	}
...
func NewHandleFromID(id ID) (*Handle, error) {
	fd, err := pkg.BPFObjGetFDByID(pkg.BPF_BTF_GET_FD_BY_ID, uint32(id))
	if err != nil {
...
func bpfLoadBTF(attr *bpfLoadBTFAttr) (*pkg.FD, error) {
	fd, err := pkg.BPF(pkg.BPF_BTF_LOAD, unsafe.Pointer(attr), unsafe.Sizeof(*attr))
	if err != nil {
		return nil, err
	}
...
var haveBTF = pkg.FeatureTest("BTF", "5.1", func() error {
	var (
		types struct {
			Integer btfType
			Var     btfType
	...
	btf := marshalBTF(&types, strings, pkg.NativeEndian)
...
var haveFuncLinkage = pkg.FeatureTest("BTF func linkage", "5.6", func() error {
	if err := haveBTF(); err != nil {
		return err
	}
